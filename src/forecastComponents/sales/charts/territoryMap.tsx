...
export default function TerritoryARRChart() {
  ...
  useEffect(() => {
    async function calculateTerritoryARR() {
      try {
        setIsLoading(true);
        ...
        // Track ARR by Territory
        const TerritoryWonARR: Record<string, number> = {};
        const TerritoryLostARR: Record<string, number> = {};
        let totalWonARR = 0;
        let totalLostARR = 0;
        ...
        accounts.forEach((account: Account) => {
          ...
          if (closedDateObj.isAfter(startDate) && closedDateObj.isBefore(endDate.add(1, 'day'))) {
            ...
            if (stage === "4 - Customer") {
              // Won deal
              TerritoryWonARR[Territory] = (TerritoryWonARR[Territory] || 0) + arr;
              totalWonARR += arr;
            } 
            else if (stage === "5a - Closed Lost") {
              // Lost deal
              TerritoryLostARR[Territory] = (TerritoryLostARR[Territory] || 0) + arr;
              totalLostARR += arr;
            }
          }
        });
        ...
        // Get all unique Territorys
        const allTerritorys = new Set<string>([
          ...Object.keys(TerritoryWonARR),
          ...Object.keys(TerritoryLostARR)
        ]);
        
        // Create chart data
        allTerritorys.forEach(Territory => {
          TerritoryData.push({
            name: Territory,
            wonARR: TerritoryWonARR[Territory] || 0,
            lostARR: TerritoryLostARR[Territory] || 0
          });
        });
        ...
        console.error("Error calculating Territory ARR:", error);
      } finally {
        setIsLoading(false);
      }
    }

    calculateTerritoryARR();
  }, []);
  ...
  if (isLoading) return <div className="flex items-center justify-center h-64">Loading Territory ARR data...</div>;
